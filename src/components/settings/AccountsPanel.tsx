...
export function AccountsPanel({ compact }: { compact?: boolean }) {
  const {
    hasData,
    snapshot,
    allocations,
  ...
  if (!hasData || !snapshot) {
    return (
      <EmptyState
        ...
        message="Import a Payroll Funding Report on Upload to manage accounts here."
      />
    );
  }
            ...
            {sorted.map((fs) => {
              ...
              return (
                <tr key={fs.id} className="border-t hover:bg-slate-50/80">
                  <td className="whitespace-nowrap px-3 py-2">
                    <AliasEditor
                      ...
                      onSave={(base) => {
                        if (isPlanned) {
                          updateSettings({
                            plannedFundingSources: (settings.plannedFundingSources ?? []).map((p) =>
                              p.id === fs.id ? { ...p, alias: base } : p
                            ),
                          });
                        } else {
                  ...
                  <td className="px-3 py-2 text-right tabular-nums">{formatCurrency(burden)}</td>
                </tr>
              );
            })}
          </tbody>
