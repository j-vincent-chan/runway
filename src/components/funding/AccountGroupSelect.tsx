...
function AccountGroupPill({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center rounded-full bg-inset px-2.5 py-1 text-xs font-medium leading-tight text-ink-2 ring-1 ring-rule",
        className
      )}
    >
      {meta.label}
...
export function AccountGroupLegend() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 text-xs text-ink-2">
      <span className="font-medium text-ink-2">Account groups</span>
      {groups.map((g) => (
        <span key={g.id} className="inline-flex items-center gap-1.5">
          <span className={cn("h-3 w-3 rounded-full ring-1 ring-rule/10", g.dotClass)} aria-hidden />
          {g.label}
        </span>
      ))}
    </div>
...
export function AccountGroupSelect({
  ...
  if (groups.length === 0) {
    return <span className="text-xs text-muted">Create groups in Settings</span>;
  }
  ...
  const menu = open
    ? createPortal(
        <div
          ref={menuRef}
          className="fixed z-50 max-h-[min(16rem,calc(100vh-1rem))] min-w-[11rem] overflow-y-auto rounded-lg border border-rule bg-surface py-1 shadow-lg"
          role="listbox"
          style={{
            top: menuPos?.top ?? 0,
            left: menuPos?.left ?? 0,
    ...
    <div ref={wrapRef} className="relative min-w-[13.5rem]">
      ...
      >
        {value ? (
          ...
          <span className="text-xs text-muted">Select account group</span>
        )}
