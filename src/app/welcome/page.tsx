      ...
      <div className="w-full max-w-lg rounded-xl bg-surface p-6 shadow-lg">
        {step === "role" ? (
          <>
            ...
            <p className="mt-1 text-sm text-ink-2">How will you use Runway?</p>
            <div className="mt-4 space-y-2">
              <button
                type="button"
                disabled={busy}
                onClick={() => void choose("pi")}
                className="flex w-full items-start gap-3 rounded-lg border border-control p-4 text-left hover:border-accent hover:bg-accent-soft/40 disabled:opacity-50"
              >
                <FlaskConical className="mt-0.5 h-5 w-5 shrink-0 text-accent" aria-hidden />
                <span>
