...
- **Notification badge.** The bell's alerts (`generateAlerts` — coverage gaps, funding cliffs, stale payroll, unaliased accounts) are a different domain from the Dashboard's own $-runway attention queue, and its "full view" link pointed to Timeline, so the badge had no explanation near the app's landing page. Later removed entirely at the owner's request: the header bell is gone and `generateAlerts` currently has no surface (`src/components/alerts/AlertsBell.tsx` remains, unmounted). If alert visibility is needed again, give it a home on the Dashboard rather than reviving the bell.
